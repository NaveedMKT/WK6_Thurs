...
uidata <- read_csv(here("data","unintentional_injuries.csv"))
```
...
```{r}
names(uidata)
falls_admissions_75_summary <- uidata %>% 
  filter(InjuryType == "Falls", 
         ...
         InjuryLocation == "Home") %>% 
  group_by(HBR) %>% 
  summarise(sum_number = sum(NumberOfAdmissions))
```
...
#It is a multipolygon geometry type
names(falls_admissions_75_summary)
names(NHS_healthboards)
uidata_joined <- full_join(falls_admissions_75_summary, NHS_healthboards, by = join_by(HBR == HBCode))
```
...
```{r}
uidata_joined %>% 
  ggplot(aes(geometry = geometry, fill = sum_number)) +
  geom_sf() +
  scale_fill_distiller() +
  labs(title = "Number of Falls", subtitle = "In Scotland")
```
...
```{r}
hbdata <- read_csv(here("data", "hb2019_pop_est.csv")) %>% 
  filter(Sex == "Female",
         Year == "2022",
         HB != "S92000003") %>%
  select(HB, Age75:Age90plus) %>% 
  mutate(sum_HBR = rowSums(across(Age75:Age90plus))) %>% 
  select(HB, sum_HBR) %>% 
  mutate(falls_per_10k = sum_HBR/10000)

#pivot_longer(-HB, names_to = "Age_Group", values_to = "n") %>% 
#summarize(Number = sum(n), .by = HB)

hbdata_joined <- full_join(hbdata, NHS_healthboards, by = join_by(HB == HBCode))

hbdata_joined %>% 
  ggplot(aes(geometry = geometry, fill = falls_per_10k)) +
  geom_sf() +
  scale_fill_distiller(palette = 2) +
  labs(title = "Number of Falls per 10k", subtitle = "In Scotland")
